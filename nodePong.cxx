...
int main() {
	...
	double prev_timer 	   = {0.};
   	double cursor_progress = {0.};

    while (!WindowShouldClose()) {
    	dt = GetFrameTime();
    	prev_timer = global_timer;
    	global_timer += dt;
    	if (fmod(global_timer, .5) < fmod(prev_timer, .5))
    		PlaySound(fx.at(5)); // kick
    	if (fmod(global_timer, .25) < fmod(prev_timer, .25))
    		PlaySound(fx.at(4));
    	cursor_progress = fmod(global_timer, 1.);
    	if (cursor_progress < fmod(prev_timer, 1.)) {
